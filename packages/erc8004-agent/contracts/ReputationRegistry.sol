...
/**
 * @title EarnbaseReputationRegistry
 * @notice Tracks reputation and feedback for Earnbase agents.
 * @dev Stores on-chain reviews from clients and task requesters.
 */
contract EarnbaseReputationRegistry is OwnableUpgradeable, UUPSUpgradeable {

    int128 private constant MAX_ABS_VALUE = 1e38;

    // --- IERC8004Reputation Events ---
    event ContributionRecorded(address indexed contributor, bytes32 indexed context, uint256 weight);
    event AgentActionRecorded(address indexed agent, bytes32 indexed context, uint256 score);
    ...
    struct ReputationRegistryStorage {
        ...
        // --- Hybrid Reputation Storage ---
        mapping(address => uint256) _contributorScores;
        mapping(address => bool) _authorizedCallers;
    }
    ...
    // --- Hybrid Reputation Logic ---

    modifier onlyAuthorized() {
        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        require(msg.sender == owner() || $._authorizedCallers[msg.sender], "Not authorized");
        _;
    }

    function setAuthorizedCaller(address caller, bool allowed) external onlyOwner {
        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        $._authorizedCallers[caller] = allowed;
    }
    ...
    function recordContribution(
        ...
        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        $._contributorScores[contributor] += weight;
        emit ContributionRecorded(contributor, context, weight);
    }
    ...
    function recordAgentAction(
        ...
        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        $._contributorScores[agent] += score; 
        emit AgentActionRecorded(agent, context, score);
    }

    function getContributorScore(address contributor) external view returns (uint256) {
        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        return $._contributorScores[contributor];
    }
